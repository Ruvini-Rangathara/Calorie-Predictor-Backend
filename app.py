...
def predict_calories():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "error": "Validation error", "details": str(e)}),
            400,
        )
            ...
            jsonify(
                {
                    ...
                    "details": (
                        str(e)
                        if app.config["DEBUG"]
                        else "An error occurred during prediction"
                    ),
                }
            ),
            500,
...
def batch_predict():
    ...
    try:
        ...
        if not data or "inputs" not in data:
            return jsonify({"error": "Expected JSON with 'inputs' array"}), 400
        ...
        for i, input_data in enumerate(inputs):
            try:
                ...
                results.append(
                    {"index": i, "success": False, "input": input_data, "error": str(e)}
                )

        return jsonify(
            {"success": True, "total_inputs": len(inputs), "results": results}
        )
